...
final class AdvancedWebViewController: NSObject {
    ...
    func newWebView(_ url: URL?, archiveURL: URL? = nil, loadMimeType: String = "application/x-webarchive") -> WKWebView {
        ...
        if isPrivateRelayEnabled {
            let privateRelayUserScript = WKUserScript(source: embeddedScripts["PrivateRelayInjection"]!,
                                                      injectionTime: .atDocumentStart,
                                                      forMainFrameOnly: false)
            wkWebView.configuration.userContentController.addUserScript(privateRelayUserScript)
        }
        if isAdBlockEnabled {
            let adblockScript = WKUserScript(source: embeddedScripts["ADBlock"]!, injectionTime: .atDocumentStart, forMainFrameOnly: false)
            wkWebView.configuration.userContentController.addUserScript(adblockScript)
        }
